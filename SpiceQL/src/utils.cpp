...
#ifdef SPICEQL_WASM
      // In the WASM build there is no host filesystem and env vars may be
      // unset. Kernels are furnished from the Emscripten virtual FS via
      // explicit paths, so fall back to the FS root instead of throwing.
      return "/";
#else
